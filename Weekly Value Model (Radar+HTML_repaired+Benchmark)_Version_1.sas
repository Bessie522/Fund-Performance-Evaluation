...
data stat;
  ...
  dfrate=(1+yfrate)**(1/52)-1;                                                 /*日化无风险收益率*/
  yprorate=(ljjz**(365/yxts))-1;                                                  /*年化收益率*/
  dprorate=((1+yprorate)**(1/52))-1;                                                    /*日化收益率*/
...
/*算出周胜率*/
proc sql;
create table weekwin1 as select count(drate) as couwwin from pardata1 where drate>0;         /*找出周次收益率为正的数量*/
run;
quit;
proc sql;
create table weekwin2 as select count(drate) as couweek from pardata1;                              /*周次收益率的总数量*/
run;
quit;
data weekwin;
...
data ddstd;
  ...
  yddstd=sqrt(sumddex/countddex)*sqrt(52);                                                               /*下行标准差*/
run;
data stat;
  set stat;
...
data hs_stat;
  ...
  hsdfrate=(1+hsyfrate)**(1/52)-1;                                                 /*日化无风险收益率*/
  hsyprorate=(hsljjz**(365/hsyxts))-1;                                                  /*年化收益率*/
  hsdprorate=((1+hsyprorate)**(1/52))-1;                                                    /*日化收益率*/
...
/*算出周胜率*/
proc sql;
create table hsweekwin1 as select count(hsdrate) as hscouwwin from hsdata1 where hsdrate>0;         /*找出周次收益率为正的数量*/
run;
quit;
proc sql;
create table hsweekwin2 as select count(hsdrate) as hscouweek from hsdata1;                              /*周次收益率的总数量*/
run;
quit;
data hsweekwin;
...
data hsddstd;
  ...
  hsyddstd=sqrt(hssumddex/hscountddex)*sqrt(52);                                                               /*下行标准差*/
run;
data hs_stat;
  set hs_stat;
...
outfile='D:\AstroInvest Files\03_自营产品\评测后数据\ANALIZED_DATA\BENCHMARK\fund_dvalue_stat-BENCHMARK.xls' 
dbms=excelcs replace;
sheet='hssumstat';
