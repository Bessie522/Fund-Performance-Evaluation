...
/*对月累计净值数据先做基本运算，算出月次累计净值、累计收益率、最大回撤*/
data parfdval;
  set parfdval;
...
data pardata1;
  set parfdval;
    drate=dval/lag(dval)-1;          /*dval是指月次累计净值*/
run;
...
data stat;
  ...
  dfrate=(1+yfrate)**(1/12)-1;                                                 /*月化无风险收益率*/
  yprorate=(ljjz**(365/yxts))-1;                                                  /*年化收益率*/
  dprorate=((1+yprorate)**(1/12))-1;                                                    /*月化收益率*/
...
/*计算离差系数*/
data stat;
  merge stat dstd;                                                 /*月波动率*/
run;
data stat;
  set stat;
  dlcxs=abs(dstd/dprorate);                                                  /*月离差系数*/
  drop dstd;
run;
...
proc sql;
create table monthwin1 as select count(drate) as coumwin from pardata3 where drate>0;         /*找出月次收益率为正的数量*/
run;
quit;
proc sql;
create table monthwin2 as select count(drate) as coumonth from pardata3;                              /*月次收益率的总数量*/
run;
quit;
data monthwin;
  merge monthwin1 monthwin2;
run;
quit;
data monthwin;
  set monthwin;
  mwinrate=coumwin/coumonth;                                  /*合并以后算出月次胜算率*/
run;
data stat;
  set stat;
  merge stat monthwin;
  drop coumwin coumonth;                                                                 /*月胜算率*/
run;
...
/*计算连续上涨月数、连续下跌月数*/
/*非连续上涨或者下跌的天数计数*/
data max_month;                                    /*连续上涨、下跌天数*/
  set pardata3(keep=drate);
  retain n 0;
  if drate > 0 then if n > 0 then  n=n+1;
            else n=1;
  if drate=0 then  n=0;
  if drate < 0 then if n < 0 then n=n-1;
                        else n=-1;
run;
proc means data=max_month max min noprint;
  var n;
  output out=rdmonth(drop=_TYPE_ _FREQ_ rename=(_STAT_=stat));
  title;
run;
proc sql;
create table max_rmonth as select n as max_rmonth from rdmonth where stat='MAX';
create table max_dmonth as select n as max_dmonth from rdmonth where stat='MIN';
run;
quit;
data max_rdmonth;
  merge max_rmonth max_dmonth;
run;
data stat;
  merge stat max_rdmonth;
run;
...
run;
proc sql;
create table max_drate as select drate as max_drate from maxwl_drate where stat='MAX';                   /*最大单月盈利*/
create table maxl_drate as select drate as maxl_drate from maxwl_drate where stat='MIN';                        /*最大单月亏损*/
run;
quit;
data max_drate;
...
run;
proc sql;
create table minl_drate as select drate as minl_drate from min_drate where stat='MIN';                        /*最大单月亏损*/
run;
quit;
data stat;
  merge stat maxl_draw minl_drate;                                                                       /*合并历史最大回撤、单月最大回撤*/
run;
...
data ddstd;
  ...
  yddstd=sqrt(sumddex/countddex)*sqrt(12);                                                               /*下行标准差*/
run;
data stat;
  set stat;
...
/*进行日次收益率的自拟合*/
data fitcurve(rename=(lagdrate=x drate=y));
  set parfdval(keep=drate);
  if drate=. then drate=0;
  lagdrate=lag(drate);
  if lagdrate=. then delete;
run;
...
proc transpose data=stat2 out=sum_stat;
  label ljjz='累计净值' yxts='运行天数' yfrate='年化无风险利率' dfrate='月化无风险利率' yprorate='年化收益率' dprorate='月化收益率' dlcxs='月离差系数' 
        mwinrate='月胜算率' max_rmonth='最大连续上涨月数' max_dmonth='最大连续下跌月数' 
        max_drate='单月最大盈利' maxl_drate='单月最大亏损' maxl_draw='历史最大回撤' minl_drate='单月最大回撤' recwnum='历史最大回撤恢复月数'
        dstd='月波动率' ylcxs='年离差系数' ystd='年化波动率' yddstd='年化下行标准差' sharp='夏普比' sortino='索提诺比' calmar='calmar值' hurst='hurst指数' score='产品打分';
run;
...
/*单独提取月次收益率、累计收益率和最大回撤，并输出至excel*/
data drate;
...
run;
proc export data=drate                                            /*输出月次收益率*/
outfile='D:\AstroInvest Files\03_自营产品\评测后数据\ANALIZED_DATA\ANALIZED\fund_dvalue_stat-ANALIZED.xls' 
dbms=excelcs replace;
sheet='drate';
run;
...
/*********************************************************
************************************************************/
/*对日累计净值数据先做基本运算，算出月次累计净值、累计收益率、最大回撤*/
data hsdval;
  set hsdval;
...
data hsdata1;
  set hsdval;
    hsdrate=hsdval/lag(hsdval)-1;          /*dval是指月次累计净值*/
run;
	/*取dval的第一个值作为算累计收益率的分母*/
data hsdata2;
  set hsdata1;
...
data hs_stat;
  ...
  hsdfrate=(1+hsyfrate)**(1/12)-1;                                                 /*月化无风险收益率*/
  hsyprorate=(hsljjz**(365/hsyxts))-1;                                                  /*年化收益率*/
  hsdprorate=((1+hsyprorate)**(1/12))-1;                                                    /*月化收益率*/
...
/*计算离差系数*/
data hs_stat;
  merge hs_stat hsdstd;                                                 /*月波动率*/
run;
data hs_stat;
  set hs_stat;
  hsdlcxs=abs(hsdstd/hsdprorate);                                                  /*月离差系数*/
  drop hsdstd;
run;
...
proc sql;
create table hsmonthwin1 as select count(hsdrate) as hscoumwin from hsdata3 where hsdrate>0;         /*找出月次收益率为正的数量*/
run;
quit;
proc sql;
create table hsmonthwin2 as select count(hsdrate) as hscoumonth from hsdata3;                              /*月次收益率的总数量*/
run;
quit;
data hsmonthwin;
  merge hsmonthwin1 hsmonthwin2;
run;
quit;
data hsmonthwin;
  set hsmonthwin;
  hsmwinrate=hscoumwin/hscoumonth;                                  /*合并以后算出月次胜算率*/
run;
data hs_stat;
  set hs_stat;
  merge hs_stat hsmonthwin;
  drop hscoumwin hscoumonth;                                                                 /*月胜算率*/
run;
...
data hsmax_week;                                    /*连续上涨、下跌天数*/
  set hsdata3(keep=hsdrate);
  retain n 0;
  if hsdrate > 0 then if n > 0 then  n=n+1;
            else n=1;
  if hsdrate=0 then  n=0;
  if hsdrate < 0 then if n < 0 then n=n-1;
                        else n=-1;
run;
...
run;
proc sql;
create table hsmax_drate as select hsdrate as hsmax_drate from hsmaxwl_drate where hs_stat='MAX';                   /*最大单月盈利*/
create table hsmaxl_drate as select hsdrate as hsmaxl_drate from hsmaxwl_drate where hs_stat='MIN';                        /*最大单月亏损*/
run;
quit;
data hsmax_drate;
...
run;
proc sql;
create table hsminl_drate as select hsdrate as hsminl_drate from hsmin_drate where hs_stat='MIN';                        /*最大单月亏损*/
run;
quit;
data hs_stat;
  merge hs_stat hsmaxl_draw hsminl_drate;                                                                       /*合并历史最大回撤、单月最大回撤*/
run;
...
data hsddstd;
  ...
  hsyddstd=sqrt(hssumddex/hscountddex)*sqrt(12);                                                               /*下行标准差*/
run;
data hs_stat;
  set hs_stat;
...
proc transpose data=hs_stat2 out=hssum_stat;
  label hsljjz='累计净值' hsyxts='运行天数' hsyfrate='年化无风险利率' hsdfrate='月化无风险利率' hsyprorate='年化收益率' hsdprorate='月化收益率' hsdlcxs='月离差系数' 
        hsmwinrate='月胜算率' hsmax_rweek='最大连续上涨月数' hsmax_dweek='最大连续下跌月数' 
        hsmax_drate='单月最大盈利' hsmaxl_drate='单月最大亏损' hsmaxl_draw='历史最大回撤' hsminl_drate='单月最大回撤' hsrecwnum='历史最大回撤恢复周数'
        hsdstd='月波动率' hsylcxs='年离差系数' hsystd='年化波动率' hsyddstd='年化下行标准差' hssharp='夏普比' hssortino='索提诺比' hscalmar='calmar值' hshurst='hurst指数';
run;
                                                                    /*在log中显示统计结果的name*/
...
/*进行日次收益率的自拟合*/
data hsfitcurve(rename=(laghsdrate=x hsdrate=y));
  set hsdval(keep=hsdrate);
  if hsdrate=. then hsdrate=0;
  laghsdrate=lag(hsdrate);
  if laghsdrate=. then delete;
run;
                                                                           /*对自拟合图形进行存放*/
...
data nh_stat;
  ...
  nhdfrate=(1+nhyfrate)**(1/12)-1;                                                 /*日化无风险收益率*/
  nhyprorate=(nhljjz**(365/nhyxts))-1;                                                  /*年化收益率*/
  nhdprorate=((1+nhyprorate)**(1/12))-1;                                                    /*月化收益率*/
...
/*计算离差系数*/
data nh_stat;
  merge nh_stat nhdstd;                                                 /*月波动率*/
run;
data nh_stat;
  set nh_stat;
  nhdlcxs=abs(nhdstd/nhdprorate);                                                  /*月离差系数*/
  drop nhdstd;
run;
...
proc sql;
create table nhmonthwin1 as select count(nhdrate) as nhcoumwin from nhdata3 where nhdrate>0;         /*找出周次收益率为正的数量*/
run;
quit;
proc sql;
create table nhmonthwin2 as select count(nhdrate) as nhcoumonth from nhdata3;                              /*周次收益率的总数量*/
run;
quit;
data nhmonthwin;
  merge nhmonthwin1 nhmonthwin2;
run;
quit;
data nhmonthwin;
  set nhmonthwin;
  nhmwinrate=nhcoumwin/nhcoumonth;                                  /*合并以后算出月次胜算率*/
run;
data nh_stat;
  set nh_stat;
  merge nh_stat nhmonthwin;
  drop nhcoumwin nhcoumonth;                                                                 /*月胜算率*/
run;
...
data nhmax_week;                                    /*连续上涨、下跌天数*/
  set nhdata3(keep=nhdrate);
  retain n 0;
  if nhdrate > 0 then if n > 0 then  n=n+1;
            else n=1;
  if nhdrate=0 then  n=0;
  if nhdrate < 0 then if n < 0 then n=n-1;
                        else n=-1;
run;
...
run;
proc sql;
create table nhmax_drate as select nhdrate as nhmax_drate from nhmaxwl_drate where nh_stat='MAX';                   /*最大单月盈利*/
create table nhmaxl_drate as select nhdrate as nhmaxl_drate from nhmaxwl_drate where nh_stat='MIN';                        /*最大单月亏损*/
run;
quit;
data nhmax_drate;
...
run;
proc sql;
create table nhminl_drate as select nhdrate as nhminl_drate from nhmin_drate where nh_stat='MIN';                        /*最大单月亏损*/
run;
quit;
data nh_stat;
  merge nh_stat nhmaxl_draw nhminl_drate;                                                                       /*合并历史最大回撤、单月最大回撤*/
run;
...
proc transpose data=nh_stat2 out=nhsum_stat;
  label nhljjz='累计净值' nhyxts='运行天数' nhyfrate='年化无风险利率' nhdfrate='月化无风险利率' nhyprorate='年化收益率' nhdprorate='月化收益率' nhdlcxs='月离差系数' 
        nhmwinrate='月胜算率' nhmax_rweek='最大连续上涨月数' nhmax_dweek='最大连续下跌月数' 
        nhmax_drate='单月最大盈利' nhmaxl_drate='单月最大亏损' nhmaxl_draw='历史最大回撤' nhminl_drate='单月最大回撤' nhrecwnum='历史最大回撤恢复月数'
        nhdstd='月波动率' nhylcxs='年离差系数' nhystd='年化波动率' nhyddstd='年化下行标准差' nhsharp='夏普比' nhsortino='索提诺比' nhcalmar='calmar值' nhhurst='hurst指数';
run;
                                                                    /*在log中显示统计结果的name*/
...
/*进行日次收益率的自拟合*/
data nhfitcurve(rename=(lagnhdrate=x nhdrate=y));
  set nhdrate(keep=nhdrate);
  if nhdrate=. then nhdrate=0;
  lagnhdrate=lag(nhdrate);
  if lagnhdrate=. then delete;
run;
                                                                           /*对自拟合图形进行存放*/
/*filename gifout "D:\AstroInvest Files\03_自营产品\评测后数据\兴鑫\同期沪深300\fitcurve-同期沪深300.gif";         /*拟合图存放于此路径下*/                                                                                         
/*goptions reset=all device=gif gsfname=gifout gsfmode=replace;*/                                                                             
